...
describe('Teste App', () => {
  ...
  test('verifica o filtro ascendente', async () => {
    const planetsSort = ['Bespin', 'Tatooine', 'Yavin IV', 'Dagobah', 'Endor', 'Naboo', 'Alderaan', 'Hoth', 'Kamino', 'Coruscant'];
    ...
    const planets = await screen.findAllByTestId('planet-name');
    expect(planets).toHaveLength(10);
    planets.forEach((item, index) => expect(item).toHaveTextContent(planetsSort[index]));
  });

  test('verifica o filtro descendente', async () => {
    const planetsSort = ['Hoth', 'Kamino', 'Alderaan', 'Naboo', 'Yavin IV', 'Dagobah', 'Endor', 'Tatooine', 'Bespin', 'Coruscant'];
